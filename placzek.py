...
def FitIncidentSpectrum(
        InputWorkspace,
        OutputWorkspace,
        FitSpectrumWith='GaussConvCubicSpline',
        Binning="0.15,0.05,3.2"):
    ...
    if FitSpectrumWith == 'CubicSpline':
        fit, fit_prime = fitCubicSpline(
            x_lambda, y_intensity, x_lo=lam_lo, x_hi=lam_hi, s=1e7)
        ...
        fit, fit_prime = fitCubicSplineViaMantidSplineSmoothing(
            InputWorkspace=InputWorkspace, MaxNumberOfBreaks=8)
        plotIncidentSpectrum(
            x_lambda,
            y_intensity,
            fit,
        ...
        fit, fit_prime = fitHowellsFunction(
            x_lambda, y_intensity, x_lo=lam_lo, x_hi=lam_hi)
        plotIncidentSpectrum(
            x_lambda,
            y_intensity,
            fit,
        ...
        spline_fit, spline_fit_prime = fitCubicSplineWithGaussConv(
            x_lambda, y_intensity, x_lo=lam_lo, x_hi=lam_hi, sigma=2)
...
def CalculatePlaczekSelfScattering(
    ...
    for species, props in atom_species.iteritems():
        elastic_term += props['concentration'] * \
            props['tot_scatt_length'] * neutron_mass / props['mass']
    ...
    for bank, (l2, theta, phi) in enumerate(zip(L2, Polar, Azimuthal)):
        ...
        per_bank_correction = 2. * \
            (term1 - term2 + term3) * sin_theta_by_2 * sin_theta_by_2 * elastic_term
