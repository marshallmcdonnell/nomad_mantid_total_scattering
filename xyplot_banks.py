...
parser.add_argument('--error', action="store_true", default=False, help='Plot error bars')
...
for i, filename in enumerate(args.filenames):
    with open(filename, 'r') as f:
        if len(args.filenames) == 1:
            plot_banks(f,title=filename,banks_list=args.banks, line_style=line_styles[i],error=args.error)
        else:
            dataset = plot_banks(f,title=filename,banks_list=args.banks, line_style=line_styles[i],error=args.error)
