...
charPM = mantid.PropertyManagerDataService.retrieve(
    '__pd_reduction_properties')

# get back the runs to use so they can be explicit in the generated python
# script
...
if mpiRank == 0:
    ...
    if sampleBackRun > 0 and not os.path.exists(canCacheName):
        ConvertUnits(
            InputWorkspace=canWkspName,
            OutputWorkspace=canWkspName,
            Target="TOF")
    ...
    if vanRun > 0 and not os.path.exists(vanCacheName):
        ConvertUnits(
            InputWorkspace=vanWkspName,
            OutputWorkspace=vanWkspName,
            Target="TOF")
